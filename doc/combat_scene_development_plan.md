...
- 상태: 다음 우선순위.
- 자동 검증 기준: `gdlint` 통과, GUT 131개 통과.
...
1. `test/manual/test_combat.tscn`에서 산적 조우를 3회 진행하고, 승리/패배/중단 여부와
   이상 로그를 기록한다.
2. Explore 화면에서 성채/폐허/거미 숲에 진입해 산적/언데드/야수 조우가 각각 로드되는지
   확인한다.
3. 같은 seed의 같은 조우를 2회 반복해 주요 공격 결과와 로그 순서가 재현되는지 확인한다.
4. 이동 후 공격, AP가 남은 상태의 추가 행동, ZoC 이탈, footwork, recover, shieldwall을
   수동 체크리스트로 확인한다.
5. 원거리 사격선, 탄약 부족, 고도 사거리 보너스, 차단 타일을 수동으로 확인한다.
6. 기절/출혈/그물/방패벽과 사기 체크가 UI와 로그에 이해 가능한 문구로 남는지 확인한다.
7. 수동 QA에서 발견한 P0 버그를 먼저 수정하고, 새 회귀 테스트를 추가한다.
8. 남은 P1 확장 후보는 `fleeing`, 부상, 시야/안개, 엄폐/오발, 전투 후 XP/전리품 순서로
   별도 작업 단위로 분리한다.
