...
핵심 상태 이상, 임시 부상, AP 기반 AI가 들어가 있다. `CombatScreen`은 이동/공격
예측 UI와 로그를 통해 현재 규칙 결과를 표시한다.
...
- `BrotherData`, 형제단 배경, 전체 적 로스터, 챔피언 변형, 특성/퍽 전체 보정,
  시야/안개, 부상 심화, 전투 후 XP/전리품 반영은 아직 구현되어 있지 않다.
...
- 완료: 상태 enum, 사망/큰 피해 이벤트 사기 체크, `fleeing` 전환과 기본 도망 AI.
- 남음: 사기 상태별 능력치 배율, `unbreakable`, Rally 계열 스킬.
...
| 2026-08-18 - 2026-08-31 | M6. 사기, 상태 이상, 부상 | P1 | 부분완료 | 사기 체크, `fleeing`, 전장 이탈, 기절/출혈/그물/방패벽, 임시 부상 완료. 부상 심화, 사기 배율 남음 |
...
- 테스트 상태: `gdlint` 통과, GUT 158개 통과.
- 이번 알파 최소판 완료: `fleeing` 전환, 공격/회복/대기 조작 제한, ZoC 비활성화,
  화면 자동 진행, 가장자리 이동 도망 AI, 가장자리 도달 시 전장 이탈 처리와 로그.
- 이번 알파 최소판 완료: `InjuryData`, `InjuryRegistry`, 큰 HP 피해 기반 임시 부상 판정,
  머리/몸 부상 분기, 부상별 능력치 패널티, 공격 결과 `injury` 필드, UI/로그 표시.
- 남은 확장: 사기 상태별 능력치 배율, 창벽/반격/불굴, 부상별 지속 피해 연계,
  부상 회복일/영구 부상, 부상 면역/확률 보정.
...
- 우선순위 5: 남은 P1 확장 후보를 부상 심화, 시야/안개, 엄폐/오발,
  전투 후 XP/전리품 순서로 재정렬.
...
- 상태: 안정화 진행 중.
- 자동 검증 기준: `gdlint` 통과, GUT 158개 통과.
...
- 완료: 공격 로그, 빗나감 로그, 사기 체크 로그, 공격 예측 UI가 핵심 결과 필드를
  표시하도록 포맷 테스트로 고정한다.
- 완료: breaking 상태에서 추가 사기 실패 시 `fleeing`으로 전환되고, `fleeing` 유닛은
  공격/회복/대기 없이 가장자리로 이동하는지 테스트로 고정한다.
- 완료: `fleeing` 유닛이 가장자리 칸에 도달하면 점유 셀, 턴 큐, 공격 대상에서 제외되고
  팀 전멸 여부에 따라 승패가 갱신되는지 테스트로 고정한다.
...
- 완료: 회복 로그와 ZoC 이탈 성공/저지/`footwork` 로그가 핵심 결과 필드를 표시하도록
  포맷 테스트로 고정한다.
- 완료: `InjuryData`/`InjuryRegistry`로 뇌진탕과 깊은 상처를 등록하고, 큰 HP 피해가
  머리/몸 부상과 능력치 패널티로 이어지는지 테스트로 고정한다.
- 완료: 유닛 정보 UI와 공격 로그가 부상 상태와 공격 결과 `injury` 필드를 표시하도록
  포맷 테스트로 고정한다.
- 완료: `test/manual/test_combat.tscn`과 `test/manual/test_explore.tscn`은 headless smoke
  ...
  정리했다.
- 완료: 20대20 전투 상태에서 AI 턴 선택과 턴 진행이 stall 없이 다음 라운드로 넘어가는지
  자동 smoke 테스트로 고정한다. 실제 입력 지연과 wall-time 측정은 수동 QA에서 계속 본다.
...
- 완료: 임시 부상과 부상별 능력치 패널티.
- 완료: AP 기반 AI.
- 완료: 산적/언데드/야수 기본 적과 조우.
- 완료: `fleeing` 전환, 조작 제한, ZoC 비활성화, 기본 도망 AI, 전장 이탈 처리.
- 남음: 부상 지속 피해 연계, 회복일, 영구 부상, 부상 면역/확률 보정.
- 남음: 별도 시야/안개 시스템.
- 남음: 엄폐, 산탄, 아군 오발.
...
- 큰 피해로 생긴 임시 부상이 로그와 UI에 표시되고 능력치에 반영되는지 확인.
...
9. 기절/출혈/그물/방패벽, 사기 체크, 임시 부상이 UI와 로그에 이해 가능한 문구로 남는지
   확인한다.
10. 완료: 20대20 자동 smoke 테스트로 AI 턴 진행이 stall 없이 다음 라운드로 넘어가는지
    확인한다.
11. 실제 20대20 전투의 입력 지연과 AI 턴 wall-time을 수동으로 측정한다.
12. 수동 QA에서 발견한 P0 버그를 먼저 수정하고, 새 회귀 테스트를 추가한다.
13. 남은 P1 확장 후보는 부상 심화, 시야/안개, 엄폐/오발, 전투 후 XP/전리품 순서로
   별도 작업 단위로 분리한다.
