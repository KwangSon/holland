...
현재 구현은 단순한 "한 행동 후 턴 종료" 모델에 가깝다. 목표 구현은 AP, 피로도,
스킬, ZoC, 명중률, 방어구/HP 피해가 결합된 전투 모델이다.

현재 유닛 데이터 구현 상태:

- `src/data/unit_registry.gd`의 플레이어 유닛은 `성직자`, `해부학자`, `궁수` 3명만
  있다.
- `src/data/unit_registry.gd`의 적 유닛은 `고블린 습격자`, `네크로맨서` 2명만 있다.
- `CombatUnit.UnitType`도 위 5종만 정의되어 있다.
- 형제단 배경, 전체 적 로스터, 챔피언 변형, 특성, 재능, 임금, 고용 비용, 출신지
  출현 조건은 아직 구현되어 있지 않다.
- 따라서 현재 기준으로는 "브라더스와 적이 전부 추가됨" 상태가 아니다.
...
| 기간 | 마일스톤 | 우선순위 | 산출물 |
| --- | --- | --- | --- |
| 2026-06-09 - 2026-06-22 | M1. 전투 도메인 재정비 | P0 | AP/피로도/턴/고도/지형 기반 규칙 |
| 2026-06-23 - 2026-07-06 | M2. 명중률과 피해 공식 | P0 | 스킬/무기 데이터, 명중, 방어구/HP 피해 |
| 2026-07-07 - 2026-07-20 | M3. 전투 화면 1차 플레이 가능 | P0 | 이동/공격/예측 UI/로그/승패 |
| 2026-07-21 - 2026-08-03 | M4. ZoC와 이니셔티브 완성 | P1 | 이탈 공격, 대기, 회복, 라운드 순서 재계산 |
| 2026-08-04 - 2026-08-17 | M5. 원거리와 시야 | P1 | 사격선, 거리 패널티, 탄약, 고도 사거리 |
| 2026-08-18 - 2026-08-31 | M6. 사기, 상태 이상, 부상 | P1 | 사기 체크, 도망, 기절/출혈/그물, 임시 부상 |
| 2026-09-01 - 2026-09-14 | M7. AI와 적 로스터 1차 | P1 | 산적/언데드/야수 기본 조우, AP 기반 AI |
| 2026-09-15 - 2026-09-28 | M8. 안정화와 전투 알파 | P0/P1 | 전투 알파 기준 통과, 회귀 테스트, 밸런스 패스 |
...
첫 작업은 M1 전체가 아니라 아래 순서대로 쪼개서 진행한다.

1. `CombatTileData`를 추가하고 `CombatBoard.setup()`이 기존 `Array[Vector2i]` 입력과
   새 타일 데이터 입력을 모두 받을 수 있게 만든다.
2. `test/unit/test_combat_board.gd`에 지형/고도 기본 테스트를 추가한다.
3. `CombatRules.get_step_ap_cost()`와 `get_step_fatigue_cost()`를 추가한다.
4. `test/unit/test_combat_rules.gd`에 평지, 거친 지형, 늪, 고도 변경 비용 테스트를
   추가한다.
5. `CombatRules.get_legal_moves()`를 AP 기반으로 바꾼다.
6. `CombatState.move_unit()`이 이동 경로 전체 비용을 소비하게 바꾼다.
7. `CombatState.start_turn()`을 추가하고 턴 시작 AP/피로도 처리를 넣는다.
8. `CombatScreen`의 이동 하이라이트와 호버 경로 비용 표시를 새 규칙에 연결한다.
9. `./godot --headless -d -s --path "$PWD" addons/gut/gut_cmdln.gd`로 전체 테스트를
   돌린다.
10. `test/manual/test_combat.tscn`에서 이동, 턴 종료, AP/피로도 UI를 수동 확인한다.

첫 번째 커밋의 완료 기준:

- 기존 전투 화면이 깨지지 않는다.
- 기존 테스트가 통과한다.
- 새 지형/고도 비용 테스트가 통과한다.
- 평지 이동, 고도 변경 이동, AP 부족 이동 실패가 모두 재현된다.
